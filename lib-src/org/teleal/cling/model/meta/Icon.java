...
public class Icon implements Validatable {
    ...
    public Icon(String mimeType, int width, int height, int depth, URI uri, String data) {
        this(
                ...
                data != null && !data.equals("") ? ByteArray.toPrimitive(new BinHexDatatype().fromString(data)) : null
        );
    }
    ...
    void setDevice(Device device) {
        // We allow the Device class to call this setter (icons are moved between devices)
        this.device = device;
    }
    ...
    public List<ValidationError> validate() {
        ...
        if (getDepth() == 0) {
            errors.add(new ValidationError(
                    getClass(),
                    "depth",
                    "Color bitmap depth is required"
            ));
        }
        if (getUri() == null) {
            errors.add(new ValidationError(
